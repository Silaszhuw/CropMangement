/** 决策记录页面：维护常年决策与当年决策记录 */
import { useCallback, useEffect, useMemo, useState } from 'react'
import { Button, DatePicker, Form, Input, Modal, Popconfirm, Select, Space, Table, Tag, Tooltip, Typography, message } from 'antd'
...
export function DecisionsPage(): React.JSX.Element {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      const matchesKeyword =
        !normalizedKeyword ||
        [item.title, item.decisionCategory, item.decisionType, item.content, item.basisSummary, item.recommendedActions, item.userFeedback, fieldName]
          .filter(Boolean)
          .some((value) => value!.toLowerCase().includes(normalizedKeyword))

      const matchesType = !typeFilter || item.decisionType === typeFilter
      const matchesStatus = !statusFilter || item.status === statusFilter
      const matchesCategory = !categoryFilter || item.decisionCategory === categoryFilter

      return matchesKeyword && matchesType && matchesStatus && matchesCategory
    })
  }, [items, keyword, typeFilter, statusFilter, categoryFilter, fieldMap])

  const categoryOptions = useMemo(
    () => Array.from(new Set(items.map((item) => item.decisionCategory))).filter(Boolean).map((value) => ({ label: value, value })),
    [items]
  )
  ...
  const columns: ColumnsType<Decision> = useMemo(
    () => [
      ...
      { title: '决策类型', dataIndex: 'decisionType', key: 'decisionType' },
  ...
  function handleCreate(): void {
    ...
    form.setFieldsValue({ decisionDate: dayjs(), decisionType: '当年决策', status: 'pending' })
    setModalOpen(true)
  }
  ...
  return (
    <Space direction="vertical" size={24} style={{ display: 'flex' }}>
      ...
      <div className="page-header">
        <div>
          <Typography.Title level={3} style={{ marginBottom: 8 }}>决策记录</Typography.Title>
          <Typography.Paragraph type="secondary" style={{ marginBottom: 0 }}>
            维护常年决策与当年决策记录，沉淀决策依据、推荐动作与用户反馈，作为后续知识复用和优化依据。
          </Typography.Paragraph>
        </div>
        <Space>
          <Tag color="processing">筛选后 {filteredItems.length} / 共 {items.length} 条</Tag>
          <Button type="primary" onClick={handleCreate}>新增决策记录</Button>
        </Space>
      </div>
      ...
      <div className="filters-bar">
        <Input
          allowClear
          placeholder="搜索标题、类别、类型、内容、依据、推荐动作、反馈"
          value={keyword}
        ...
        <Select
          ...
          options={[{ label: '常年决策', value: '常年决策' }, { label: '当年决策', value: '当年决策' }]}
          style={{ width: 160 }}
        />
        <Select
          allowClear
      ...
      <Modal destroyOnHidden open={modalOpen} title={editingRecord ? '编辑决策记录' : '新增决策记录'} okText={editingRecord ? '保存修改' : '创建记录'} cancelText="取消" confirmLoading={submitting} onCancel={() => setModalOpen(false)} onOk={() => void form.submit()}>
        <Form layout="vertical" form={form} initialValues={{ decisionDate: dayjs(), decisionType: '当年决策', status: 'pending' }} onFinish={(values) => void handleSubmit(values)}>
          ...
          <Form.Item label="决策类型" name="decisionType" rules={[{ required: true, message: '请选择决策类型' }]}>
            <Select options={[{ label: '常年决策', value: '常年决策' }, { label: '当年决策', value: '当年决策' }]} />
          </Form.Item>
          <Form.Item label="决策类别" name="decisionCategory" rules={[{ required: true, message: '请输入决策类别' }]}>
            <Input placeholder="例如：品种选择、施肥方案、灌溉方案" />
          </Form.Item>
