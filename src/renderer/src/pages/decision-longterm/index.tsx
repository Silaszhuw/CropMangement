/** 常年决策子系统页面：作为二级工作台进入适宜生育进程模拟和决策档案 */
...
export function DecisionLongtermPage(): React.JSX.Element {
  ...
  const categoryOptions = useMemo(
    () => Array.from(new Set(items.filter(i => i.decisionType === '常年决策').map((item) => item.decisionCategory))).filter(Boolean).map((value) => ({ label: value, value })),
    [items]
  )
  ...
  return (
    <Space direction="vertical" size={24} style={{ display: 'flex' }}>
      ...
      <div className="page-header">
        ...
        <Space>
          <Tag color="processing">筛选后 {filteredItems.length} / 共 {items.filter(i => i.decisionType === '常年决策').length} 条</Tag>
          <Button type="primary" onClick={handleCreate}>新增常年决策</Button>
        </Space>
      </div>
      ...
      <div className="module-grid">
        <Card
          ...
          title="适宜生育进程模拟"
          extra={
            <Button type="link" onClick={() => navigate('/decision-longterm/simulation')}>
              进入三级界面
            </Button>
          }
        >
          <Space direction="vertical" size={12} style={{ display: 'flex' }}>
            <Typography.Paragraph style={{ marginBottom: 0 }}>
              基于常年温度背景、地块与品种参数，对关键生育阶段的适宜进程进行模拟，为播期规划和品种选择提供依据。
            </Typography.Paragraph>
            <Button type="primary" onClick={() => navigate('/decision-longterm/simulation')}>
              打开适宜生育进程模拟
            </Button>
          </Space>
        </Card>

        <Card className="module-entry-card" title="常年决策档案">
          <Typography.Paragraph style={{ marginBottom: 0 }}>
            下方表格保留播种前规划决策记录，用于沉淀品种选择、播期安排和施肥方案等长期经验。
          </Typography.Paragraph>
        </Card>
      </div>
      ...
      <Table rowKey="id" loading={loading || fieldsState.loading} columns={columns} dataSource={filteredItems} />

      <Modal destroyOnHidden open={modalOpen} title={editingRecord ? '编辑常年决策' : '新增常年决策'} okText={editingRecord ? '保存修改' : '创建记录'} cancelText="取消" confirmLoading={submitting} onCancel={() => setModalOpen(false)} onOk={() => void form.submit()}>
        <Form layout="vertical" form={form} initialValues={{ decisionDate: dayjs(), status: 'pending' }} onFinish={(values) => void handleSubmit(values)}>
          ...
          <Form.Item label="决策类别" name="decisionCategory" rules={[{ required: true, message: '请输入决策类别' }]}>
            <Input placeholder="例如：品种选择、播期规划、施肥方案" />
          </Form.Item>
